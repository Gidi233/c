...
- 实现用户在线时,消息的实时通知
	...
	- 收到加群申请
	  ...
	  方案：多开一个线程，接收到udp信息（应该是一个类），判断与当前打开的窗口是否相同，不同就解析输出
...
在收到消息时，发出信号，发出选项等待回应时抛弃信号，信号处理程序是实时通知
...
已解决：最好不要用函数返回值初始化静态（成员）变量

// flag = ~flag;有问题，按1字节按位取反
